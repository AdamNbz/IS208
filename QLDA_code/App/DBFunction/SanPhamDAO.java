...
public class SanPhamDAO {
    ...
    public boolean themSanPham(SanPham sp) {
        String sql = "INSERT INTO SANPHAM (MASP, TENSP, LOAISP, DONVITINH, TONKHO) VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, sp.getMaSP());
            stmt.setString(2, sp.getTenSP());
            stmt.setString(3, sp.getLoaiSP());
            stmt.setString(4, sp.getDonViTinh());
            stmt.setInt(5, sp.getTonKho());
    ...
    public boolean suaSanPham(SanPham sp) {
        String sql = "UPDATE SANPHAM SET TENSP = ?, LOAISP = ?, DONVITINH = ?, TONKHO = ? WHERE MASP = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, sp.getTenSP());
            stmt.setString(2, sp.getLoaiSP());
            stmt.setString(3, sp.getDonViTinh());
            stmt.setInt(4, sp.getTonKho());
